...
$params = [];
if ($date_filter) {
    $where = 'WHERE DATE(sales.sale_date) = ?';
    $params[] = $date_filter;
} elseif ($month_filter) {
    $where = 'WHERE DATE_FORMAT(sales.sale_date, "%Y-%m") = ?';
    $params[] = $month_filter;
}
$stmt = $pdo->prepare('SELECT sales.id, clients.name AS client_name, users.username AS vendeur, sales.sale_date, sales.total FROM sales LEFT JOIN clients ON sales.client_id = clients.id LEFT JOIN users ON sales.user_id = users.id ' . $where . ' ORDER BY sales.sale_date DESC');
$stmt->execute($params);
$sales = $stmt->fetchAll();
